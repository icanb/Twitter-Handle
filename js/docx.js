  ...
  var generate = function() {
    ...
    for(var file in files) {
      ...
      if (files[file] == 'word/document.xml') {
        ...
        $.ajax({
          url: 'js/blank/' + files[file],
          complete: function(r) {
            //file_data[this.url.replace(/blank_/, '')] = r.responseText;
            //console.log(r);
            zip.add(this.url.replace('js/blank/', ''), r.responseText);
            file_count_current ++;
  ...
  return {
    output: function(type, options) {
      ...
      if(type == 'datauri') {
        ...
        console.log(type)
        var outputFile = finalFile(buffer);
        document.location.href = 'data:application/vnd.openxmlformats-officedocument.wordprocessingml.document;base64,' + type;
      }
